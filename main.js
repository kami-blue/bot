...
client.on('message', async message => {
    ...
    if (message.author.id === "737399633172758608" && /.*563138570953687061.*/.test(message.content.toLowercase())) {
        message.delete()
...
async function autoResponder(message) {
    ...
    if (!message.member.hasPermission("MANAGE_MESSAGES")) {
        ...
        /* slurs regex */
        if (slursRegex.test(cleanedMessage.replace(/mini.{0,2}game/, "").replace("nigeria", "").replace("son i", "").replace("nibar", "").replace("cannibal", "").replace("fortran", ""))) {
